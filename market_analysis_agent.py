...
def extract(state: State) -> dict[str, Any]:
    ...
    prompt: ChatPromptTemplate = ChatPromptTemplate.from_template(
        ...
        # Immediate Goal: Extract the core product/topic to pass to subsequent search function.
        # Overall Goal: Conduct comprehensive market research on automotive products and trends, then format the results in a standardized format for further processing.
        # Guideline:
        ...
        Question: {question}
        You MUST ONLY return the product or topic without any additional text
        '''
    )
...
def search_tavily(state: State) -> dict[str, Any]:
    ...
    for search_query in search_queries:
        ...
        api_response: dict[str, Any] = client.search(query=query)
...
def generate(state: State) -> dict[str, Any]:
    ...
    prompt: ChatPromptTemplate = ChatPromptTemplate.from_template(
        '''
        # Role: Market Analyst for AwsomeCar
        # Immediate Goal: Provide comprehensive market research based on search engine results pages for {product} and format the results in a standardized format that can be stored for further processing.
        # Guideline:
        1. CRITICAL: For EACH data point, statistic, or claim in your analysis, you MUST include the reference source
        # Standardized Format:
        After completing your research, you MUST format your findings in the following standardized format:
        ```
        ==================================================
        ...
        ==================================================
        ```
        Search Engine Result Pages: {api_responses} 
        Standardized Research Result:
        '''
    )
    messages: PromptValue = prompt.invoke({'product': state['product'], 'api_responses': state['api_responses']})
    ai_message: BaseMessage = llm.invoke(messages)
...
def store_results_in_s3(state: State) -> dict[str, Any]:
    ...
    s3_client.put_object(Bucket=S3_BUCKET, Key=key, Body=state['answer'])
    return {'s3_result_location': f's3://{S3_BUCKET}/{key}'}
